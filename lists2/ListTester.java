...
public class ListTester {
	...
	private void runTests() {
		...
		testEmptyList(A_IterRemoveA_A, "A_IterRemoveA_A");
...
//			break;
	...
	// Tests for scenarios ending in [A]
	...
	//Test for scenarios ending in [B]
	...
	/** Scenario: [A,B] -> it = list.iterator it.next it.remove  -> [B] 
	 * @return [B] after next and remove 
	 */
	private IndexedUnsortedList<Integer> AB_IterRemoveA_B() {
		IndexedUnsortedList<Integer> list = ABC_remove2_AB(); 
	...
	/** Scenario: [A,B] -> it = list.iterator it.next it.next it.remove  -> [A] 
	 * @return [A] after next, next, and remove 
	 */
	...
	//Test for scenarios ending in [B,C]

	/** Scenario: [A,B,C] -> it = list.iterator it.next it.remove  -> [B,C] 
	 * @return [B,C] after next and remove
	 */
	...
	//Test for scenarios ending in [B,A]
	...
	//Tests for scenarios ending in [A,B]

	/** Scenario: [A,B,C] -> it = list.iterator it.next it.next it.next it.remove  -> [A,B] 
	 * @return [A,B] after next, next, next, and remove
	 */
	...
	/**
	 * Scenario: [AB] -> set(0,C) -> [C,B]
	 * @return [A,C] after set(0,C)
	 */
	...
	/** Scenario: [A,B,C] -> it = list.iterator it.next it.next it.remove  -> [A,C] 
	 * @return [A,C] after next, next, and remove
	 */
	...
	// Tests for scenarios ending in [CAB]

	/**
	 * Scenario: [AB] -> addToFrontC -> [C,A,B]
	...
	//Test for scenarios ending in [A,B,C]

	/**
	 * Scenario: [AB] -> addToRearC -> [A,B,C]
	 * @return [C,A,B] after addToRear(C)
	 */
	...
	private Scenario<Integer> AB_addToRear_ABC = () -> AB_addToRear_ABC();

	/**
	 * Scenario: [AB] -> addAfterCB -> [A,B,C]
	...
	/**
	 * Scenario: [AB] -> addAfter(C,A) -> ACB
	 * @return [ABD] after set(2,D)
	 */
	...
	//Tests for scenarios ending in []

	/** Scenario: [A] -> it = list.iterator it.next it.remove  -> [] 
	 * @return [] after next and remove 
	 */
	private IndexedUnsortedList<Integer> A_IterRemoveA_A() {
		IndexedUnsortedList<Integer> list = emptyList_addToFrontA_A(); 
		Iterator<Integer> it = list.iterator();
		it.next();
		it.remove();
	...
	private Scenario<Integer> A_IterRemoveA_A = () -> A_IterRemoveA_A();
	...
	/**
	 * Scenario: [ABC] -> set(1,D) -> ADC
	 * @return [ABD] after set(1,D)
	 */
	...
	/**
	 * Scenario: [ABC] -> set(0,D) -> DBC
	 * @return [ABD] after set(0,D)
	 */
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			//Using an overloaded version of interAfterNext that takes the return type of iterAfterRemove for all NextRemoveNext
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_testGet0", testGet(scenario.build(), 3, null, Result.IndexOutOfBounds));
			printTest(scenarioName + "_testIndexOf" + contentsString.charAt(0), testIndexOf(scenario.build(), contents[0], 0));
			printTest(scenarioName + "_testIndexOf" + contentsString.charAt(1), testIndexOf(scenario.build(), contents[1], 1));
			...
			printTest(scenarioName + "_testRemove0", testRemoveIndex(scenario.build(), 3, null, Result.IndexOutOfBounds));
			// Iterator
			printTest(scenarioName + "_testIter", testIter(scenario.build(), Result.NoException));
			printTest(scenarioName + "_testIterHasNext", testIterHasNext(scenario.build().iterator(), Result.True));
			printTest(scenarioName + "_testIterNext", testIterNext(scenario.build().iterator(), contents[0], Result.MatchingValue));
			...
			printTest(scenarioName + "_iterNextNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			//Using an overloaded version of interAfterNext that takes the return type of iterAfterRemove for all NextNextRemoveNext
	...
	/**
 	* Helper for testing iterators. Return an Iterator that has been advanced numCallsToNext times.
 	* @param list The iterator that matches the return value of iterAfterRemove
 	* @param numCallsToNext
 	* @return The iterator passed in after using next.
 	*/
	private Iterator<Integer> iterAfterNext(Iterator<Integer> list, int numCallsToNext) {
    	for (int i = 0; i < numCallsToNext; i++) {
        	list.next();
    	}
    	return list;
